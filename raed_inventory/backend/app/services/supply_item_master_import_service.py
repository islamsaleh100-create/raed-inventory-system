...
def _load_rows(workbook_path: Path) -> tuple[list[dict[str, Any]], list[tuple[int, str]]]:
    ...
    for excel_row_no, raw in enumerate(ws.iter_rows(min_row=2, values_only=True), start=2):
        ...
        if source_type == SupplySourceType.BOTH and default_source is None:
            errors.append((excel_row_no, "BOTH items require default_source"))
            continue
        if source_type == SupplySourceType.KITCHEN and not kitchen_section:
            errors.append((excel_row_no, "Kitchen item requires kitchen_section_id"))
            continue
        if source_type == SupplySourceType.WAREHOUSE and default_source == SupplyDefaultSource.KITCHEN:
            errors.append((excel_row_no, "WAREHOUSE item cannot default to KITCHEN"))
            continue
        if source_type == SupplySourceType.KITCHEN and default_source == SupplyDefaultSource.WAREHOUSE:
            errors.append((excel_row_no, "KITCHEN item cannot default to WAREHOUSE"))
            continue
        ...
        rows.append(
            {
                ...
                "default_source": default_source or SupplyDefaultSource.WAREHOUSE,
...
def _get_or_create_kitchen_section(db: Session, section_name: str | None) -> KitchenSection | None:
    if not section_name:
        return None
    ...
    if row:
        row.active = True
        return row
    row = KitchenSection(name=section_name, active=True)
    db.add(row)
    db.flush()
    return row
...
def _write_invalid_log(rejected_rows: list[dict[str, Any]], output_dir: Path) -> str:
    ...
    path = output_dir / "classified_supply_items_invalid_rows.json"
    path.write_text(json.dumps(rejected_rows, ensure_ascii=False, indent=2), encoding="utf-8")
    return str(path)
...
def import_supply_item_master(
    ...
    *,
    ...
    for row in rows:
        try:
            with db.begin_nested():
                ...
                kitchen_section = _get_or_create_kitchen_section(db, row["kitchen_section_name"])
    ...
    invalid_path = _write_invalid_log(
        rejected_rows,
        Path(invalid_log_dir or Path(__file__).resolve().parents[2] / "uploads" / "import_logs"),
    )
    return ImportResult(
